...
void nz(string &s) {
	...
	char nxt='0';
	for(char c:s) mp[c]=0;
	for(char c:s) if(isdigit(c) && !mp[c]) mp[c]=nxt++;
	for(char &c:s) if(isdigit(c)) c=(char)mp[c];
	Wpop;
}
...
void _main() { /* CURSOR START */
	...
	static map<pair<string,string>,string> dp[69][48];
	dp[0][0][{g[1].substr(0,1)+g[0],string("0123456789").substr(0,n+2)}]="";
	dbg(dp[0][0]);
	g.pb("+");
	for(int x=0;x<n;x++) {
		for(int y=0;y<=n;y++) {
			for(const auto &[k,v]:dp[x][y]) {
				auto [r,c]=k;
				if(y==n) {
					if(r[n+1]!='0' && r[n+1]!='+') continue;
					r=r.substr(0,n+1);
					c=c.substr(0,n+1);
					nz(c);
					auto rr=g[x+2].substr(0,1)+r;
					auto cc=string("9")+c;
					nz(cc);
					dp[x+1][0][{rr,cc}]=v+string("\n");
					continue;
				}

				int poss=3;
				if(r[y+1]=='+');
				else if(r[y+1]=='0') poss&=~1;
				else if(r[y+1]=='1') poss&=~2;
				else poss=0;
				if(c[y]==c[y+2]) poss&=~2;
				if(r[y]=='0') poss&=~2;
				if(r[y+2]=='0') poss&=~2;
				if(g[x+1][y+1]=='0') poss&=~1;
				...
				if(poss&1) {
					...
					if(rr[y+1]!='+') rr[y+1]--;
					auto kk=pair{rr,c};
					if(dp[x][y+1].count(kk)) dp[x][y+1][kk]="mult";
					else dp[x][y+1][kk]=v+"\\";
				}
				if(poss&2) {
					auto rr=r;
					...
					if(rr[y+2]!='+') rr[y+2]--;
					auto cc=c;
					char o=cc[y],oo=cc[y+2];
					assert(o!=oo);
					for(char &d:cc) if(d==o) d=oo;
					nz(cc);
					cc[y+1]='9';
					nz(cc);
					auto kk=pair{rr,cc};
	...
	for(const auto &[k,v]:dp[n][0]) {
		for(char c:k.fi) if(c!='0' && c!='+') goto nope;
		assert(!gg); gg=1;
		assert(v.substr(0,4)!="mult");
		cout<<v<<flush;
		nope:;
	}
}
