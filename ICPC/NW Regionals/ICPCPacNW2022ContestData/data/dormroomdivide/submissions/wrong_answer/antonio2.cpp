...
template<class T> T xx(const complex<T> &a,const complex<T> &b) { return imag(a*conj(b)); }
...
void _main() { /* CURSOR START */
	...
	forenum(i,auto &z:poly) {
		if(inp) { int x,y; cin>>x>>y; z={x,y}; }
		else { z=polar(1e7*irand(.999,1.),i*U_03C4_/n+1e-6*irand(0.,1.)); }
	}

	for(int i=0;i<5;i++) poly.pb(poly[i]);
	for(int i=1;i<=n;i++) assert(xx(poly[i-1]-poly[i],poly[i+1]-poly[i])>0);
	auto off=poly[0];
	for(auto &p:poly) p-=off;

	ll Atot=0;
	for(int i=2;i<n;i++) Atot+=xx(poly[i],poly[i-1]);

	int m=1;
	ll Acur=0,Anxt;
	for(;;) {
		ll ex=xx(poly[m+1],poly[m]);
		Anxt=Acur+ex;
		if(2*Anxt>Atot) break;
		++m, Acur=Anxt;
	}
	assert(Acur*2<=Atot && Anxt*2>Atot);

	ll num=Atot-Acur*2;
	ll den=Anxt*2-Acur*2;
	auto za=poly[m],zb=poly[m+1];
	za+=off, zb+=off;
	// flt zx=real(za)+flt(num)*real(zb-za)/den;
	// flt zy=imag(za)+flt(num)*imag(zb-za)/den;
	auto z=den*za+num*(zb-za);
	flt zx=flt(real(z))/den;
	flt zy=flt(imag(z))/den;
	cout<<fixed<<setprecision(9)<<(ld)zx<<' '<<(ld)zy<<endl;
}
