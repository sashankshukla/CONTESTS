...
void _main() { /* CURSOR START */
	int n; ine(n,arg1); assert(inb(n,3,25));
	vector<int> a(n); for(int &x:a) inr(x,1<<25), assert(inb(x,1,1<<25));

	static int req[25][25];
	bool fnd=0;
	forenum(i,int x:a) forenum(j,int y:a) forenum(k,int z:a) if(!(x^y^z) && i!=j && j!=k && k!=i) {
		req[i][j]|=1<<k; swap(i,j), swap(j,k);
		req[i][j]|=1<<k; swap(i,j), swap(j,k);
		req[i][j]|=1<<k; swap(i,j), swap(j,k);
		fnd=1;
	}
	...
	for(int msk=0;msk<(1<<n);msk++) {
		int z=popcount((unsigned)msk);
		if(z>=Z) continue;
		for(int i=0;i<n;i++) if(!(msk&1<<i)) for(int j=0;j<i;j++) if(!(msk&1<<j)) if(req[i][j]&~msk) goto nope;
		Z=z;
		nope:;
	}
	assert(Z<inf);
